...
class UserServiceImplTest {
    ...
    void updateUserById() {
        ...
        UpdateUserRequest userUpdate = new UpdateUserRequest(userDto.getId(), userDto.getEmail(), null, userDto.getName(), null);

        when(userRepository.save(any(User.class))).thenReturn(UserMapper.toUser(userDto));
        when(userRepository.findById(anyLong())).thenReturn(Optional.of(UserMapper.toUser(userDto)));
        UserDto updateUser = userService.updateUserById(userUpdate);
